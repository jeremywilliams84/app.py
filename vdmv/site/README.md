...
Les pages HTML ont été générées à l'origine par `../build_site.py` à partir de `../metiers_data.json`.

**Mise à jour (juillet 2026)** : le contenu éditorial des 15 pages métiers (héro, article, plan latéral, métadonnées SEO) est désormais régénéré par `../build_metiers.py` à partir de `../metiers_data.json` (champs spécifiques par métier : enjeux locaux, intentions de recherche, services, structure de page type, preuves, erreurs, CTA) et `../metiers_base.json` (données d'origine : objectifs, requêtes, FAQ). Chaque page porte un contenu propre à sa profession — plus aucun bloc éditorial générique où seul le nom du métier change.
